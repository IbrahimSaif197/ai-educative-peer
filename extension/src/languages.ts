...
export const SUPPORTED_LANGUAGES: Record<string, LanguageInfo> = {
  ...
  java: {
    ...
    importRegex: /^\s*(import\s+(static\s+)?[\w.*]+\s*;|package\s+[\w.]+\s*;)/,
    lineComment: "//",
    blockComment: ["/*", "*/"],
  },
  c: {
    ...
    importRegex: /^\s*#\s*(include|define|pragma|ifndef|ifdef|endif)\b/,
    lineComment: "//",
    blockComment: ["/*", "*/"],
  },
  cpp: {
    ...
    importRegex: /^\s*(#\s*(include|define|pragma|ifndef|ifdef|endif)\b|using\s+namespace\b)/,
    lineComment: "//",
    blockComment: ["/*", "*/"],
  },
  csharp: {
  ...
  go: {
    ...
    importRegex: /^\s*(import\b|package\s+\w|"[\w./-]+"\s*$|\)\s*$)/,
    lineComment: "//",
    blockComment: ["/*", "*/"],
  },
  rust: {
    ...
    importRegex: /^\s*((pub\s+)?(use|mod)\s+\w|extern\s+crate\b)/,
    lineComment: "//",
    blockComment: ["/*", "*/"],
  },
  sql: {
