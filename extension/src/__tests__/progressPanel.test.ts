...
describe("calibration", () => {
  it("asks for more samples below the threshold", () => {
    const html = buildProgressHtml(
      report({
        calibration: {
          samples: 2,
    ...
    expect(html).toContain("(2 so far)");
  });

  it("reports the score once there is enough data", () => {
    const html = buildProgressHtml(
      report({
        calibration: {
          samples: 8,
    ...
    expect(html).toContain("8 rated questions");
  });

  it("handles an older backend that sends no calibration at all", () => {
    expect(() => buildProgressHtml(report())).not.toThrow();
    expect(buildProgressHtml(report())).toContain("Rate how sure you are");
  });
});
